...
from visualization.style import configure_matplotlib  # noqa: E402
...
def _inject_design_system() -> None:
    ...
    st.markdown(
        ...
        @import url('https://fonts.googleapis.com/css2?family=DM+Mono:wght@400;500&family=Manrope:wght@400;500;600;700;800&family=Playfair+Display:wght@500;600&display=swap');
         :root { --ink:#eefcf5; --muted:#9fb8b2; --line:rgba(174,246,207,.18); --paper:#07131a; --blue:#79e6b0; --softblue:rgba(95,230,160,.12); --green:#5fe6a0; --wood:#c79bff; }
        html { text-rendering:optimizeLegibility; }
         .stApp { background:radial-gradient(circle at 15% 0%,rgba(95,230,160,.12),transparent 28%),radial-gradient(circle at 85% 12%,rgba(199,155,255,.13),transparent 24%),linear-gradient(180deg,#050b12 0%,#08151b 45%,#061017 100%); color:var(--ink); font-family:'Manrope',sans-serif; }
        #MainMenu, footer { visibility:hidden; }
        [data-testid='stHeader'] { visibility:visible; background:transparent; }
        [data-testid='stToolbar'] { visibility:hidden; }
        [data-testid='stExpandSidebarButton'] { visibility:visible!important; z-index:1000; }
        ...
        h1,h2,h3 { font-family: 'Manrope', sans-serif !important; letter-spacing:-.045em; color:var(--ink); }
         .landing-nav { position:sticky; top:0; z-index:10; display:flex; align-items:center; justify-content:space-between; background:rgba(5,12,18,.78); backdrop-filter:blur(18px); border:1px solid var(--line); border-radius:18px; padding:12px 16px; margin-bottom:28px; box-shadow:0 14px 34px rgba(0,0,0,.25); }
         .brand { font-weight:800; letter-spacing:-.05em; font-size:20px; color:#eefcf5; } .brand-dot{color:var(--green)}
        .nav-links { color:#495d63; font-size:12px; font-weight:600; letter-spacing:.01em; display:flex; gap:22px; }
         .eyebrow { color:var(--green); font:500 11px 'DM Mono',monospace; text-transform:uppercase; letter-spacing:.12em; margin-bottom:18px; }
        ...
        .impact-hero { min-height:680px; position:relative; overflow:hidden; display:flex; align-items:center; justify-content:center; text-align:center; border:1px solid rgba(255,255,255,.24); border-radius:30px; background:linear-gradient(135deg,#0b2938 0%,#174859 58%,#3d6659 100%); box-shadow:0 24px 56px rgba(15,40,52,.20); }
        ...
        .sun-disc { position:absolute; right:10%; top:11%; width:210px; height:210px; border-radius:50%; background:radial-gradient(circle at 34% 32%,#fff8e9,#e9c38d 62%,#c48e58); box-shadow:0 0 32px rgba(255,227,173,.28); z-index:1; opacity:.92; }
        ...
        .glass-research-card { position:absolute; left:7%; bottom:10%; width:215px; min-height:126px; z-index:4; padding:18px; text-align:left; border-radius:16px; background:rgba(16,48,59,.91); border:1px solid rgba(255,255,255,.58); box-shadow:0 14px 30px rgba(5,27,38,.22); color:#f4f8f5; }
        .glass-research-card b{display:block;font:500 10px 'DM Mono',monospace;letter-spacing:.1em;color:#e4efe7;margin-bottom:15px}.glass-research-card span{display:block;font-size:13px;font-weight:500;line-height:1.5}.mini-bars{display:flex;align-items:flex-end;gap:5px;height:25px;margin-top:12px}.mini-bars i{display:block;width:12px;background:#efcd94;border-radius:4px 4px 0 0}.mini-bars i:nth-child(1){height:9px}.mini-bars i:nth-child(2){height:22px}.mini-bars i:nth-child(3){height:15px}.mini-bars i:nth-child(4){height:25px}.mini-bars i:nth-child(5){height:18px}
        ...
        .hero-center h1 { color:#fff!important; font-family:'Playfair Display',serif!important; font-size:clamp(48px,7.2vw,104px); font-weight:600; white-space:nowrap; line-height:.9; margin:12px 0 20px; letter-spacing:-.075em; text-shadow:0 2px 5px rgba(7,28,38,.42); }
        .hero-center p { max-width:590px; margin:0 auto; padding:12px 18px; border:1px solid rgba(255,255,255,.32); border-radius:12px; background:rgba(8,37,46,.78); font-size:17px; font-weight:600; line-height:1.65; color:#fff; text-shadow:0 1px 2px rgba(7,28,38,.4); }
        .hero-flag { display:inline-block; background:rgba(12,43,53,.78); border:1px solid rgba(255,255,255,.55); border-radius:99px; color:#fff3df; padding:7px 12px; font:500 10px 'DM Mono',monospace; letter-spacing:.12em; }
        ...
        .hero-art:before { content:''; position:absolute; inset:-8%; background:radial-gradient(circle at 18% 30%,rgba(184,138,88,.22),transparent 24%),radial-gradient(circle at 78% 22%,rgba(61,115,87,.18),transparent 27%),radial-gradient(circle at 68% 78%,rgba(255,255,255,.42),transparent 28%); }
        .art-window { position:absolute; width:68%; height:51%; top:10%; left:13%; border:1px solid rgba(255,255,255,.9); background:rgba(255,255,255,.48); border-radius:25px; transform:rotate(-7deg); }
        ...
        .floating-label { position:absolute; bottom:8%; right:8%; border:1px solid #d4dfd8; background:#f9fcfa; border-radius:12px; padding:12px 14px; color:#315c47; font:500 10px 'DM Mono',monospace; box-shadow:0 8px 20px rgba(48,79,61,.10); }
        .section { padding:94px 7% 34px; margin:44px 0 22px; border:1px solid var(--line); border-radius:28px; background:linear-gradient(135deg,rgba(9,27,34,.94),rgba(12,24,39,.84)); box-shadow:0 18px 50px rgba(0,0,0,.22); } .section-title { font-size:clamp(32px,4vw,54px); line-height:1.05; margin:0 0 16px; max-width:760px; } .section-copy { color:var(--muted); max-width:610px; line-height:1.75; }
        .glass-card { min-height:220px; border:1px solid var(--line); background:rgba(9,25,32,.78); border-radius:20px; padding:25px; box-shadow:0 12px 30px rgba(0,0,0,.2); transition:transform .25s ease,box-shadow .25s ease; } .glass-card:hover { transform:translateY(-4px); box-shadow:0 18px 38px rgba(95,230,160,.12); } .card-icon { font-size:19px; color:var(--green); margin-bottom:42px; } .glass-card h3{font-size:19px;margin:0 0 10px}.glass-card p{font-size:13px;color:var(--muted);line-height:1.65;margin:0}
        ...
        .quote-block { padding:130px 8% 120px; text-align:center; border-radius:34px; background:radial-gradient(circle at 50% 0%,rgba(95,230,160,.16),transparent 52%),linear-gradient(135deg,#0a232b,#0d1b2d); border:1px solid var(--line); } .quote-block p{font:500 clamp(32px,4.5vw,64px)/1.12 'Playfair Display',serif;letter-spacing:-.045em;color:#dfffee;margin:0}.quote-block span{display:block;color:#8db8a8;font:500 11px 'DM Mono',monospace;letter-spacing:.12em;text-transform:uppercase;margin-top:26px}
        ...
        .stButton>button { border:1px solid rgba(174,246,207,.25)!important; border-radius:999px!important; font-family:'Manrope',sans-serif!important; font-weight:700!important; padding:.72rem 1.15rem!important; background:linear-gradient(135deg,#5fe6a0,#2f9d74)!important; color:#04110d!important; box-shadow:0 8px 22px rgba(95,230,160,.18)!important; transition:transform .2s ease,filter .2s ease!important; } .stButton>button:hover{transform:translateY(-2px);filter:brightness(1.08)}.secondary-button .stButton>button{background:rgba(95,230,160,.1)!important;color:#aef6cf!important}
        [data-testid='stSidebar'] { background:rgba(4,12,18,.94); border-right:1px solid var(--line); } [data-testid='stSidebar'] .block-container{padding:1.6rem 1rem;} [data-testid='stSidebar'] .stRadio label{font-size:13px;color:#9fb8b2;padding:7px 3px;}
        .dashboard-top { padding:12px 0 28px; } .dashboard-top h1{font-size:42px;margin:0 0 6px}.dashboard-top p{color:#5f6e82;margin:0}.prototype-pill{display:inline-block;color:#3560a8;background:#edf4ff;border:1px solid #d9e7ff;border-radius:99px;padding:6px 9px;font:500 10px 'DM Mono',monospace;margin-bottom:13px}
        ...
        .module-card{border:1px solid var(--line);background:rgba(9,25,32,.82);border-radius:20px;padding:22px;min-height:175px}.module-card h3{font-size:18px;margin:16px 0 8px}.module-card p{font-size:12px;color:var(--muted);line-height:1.65}.module-tag{font:500 10px 'DM Mono',monospace;color:#aef6cf}.quiet-note{border-left:3px solid #5fe6a0;background:rgba(95,230,160,.08);padding:14px 16px;color:#b9d8cc;font-size:12px;border-radius:0 12px 12px 0}
...
def main() -> None:
    ...
    if "view" not in st.session_state or requested_view == "platform":
        st.session_state["view"] = "platform" if requested_view == "platform" else "landing"
